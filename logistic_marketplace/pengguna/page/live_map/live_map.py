...
def get_position(principle,vendor,driver,driver_status):
	...
	if driver_status=="Idle":
		if principle!="All":
			return "No Data"
		result = frappe.db.sql("""select "Idle" as "job_order", d.last_update,d.lo,d.lat ,"None" as "principle",d.vendor ,d.name as "driver"
			from `tabDriver` d
		...
		result = frappe.db.sql("""select ifnull(p.name,"Idle") as "job_order" , d.last_update,d.lo,d.lat,ifnull(p.principle,"None") as "principle" ,d.vendor ,d.name as "driver"
			from `tabDriver` d 
