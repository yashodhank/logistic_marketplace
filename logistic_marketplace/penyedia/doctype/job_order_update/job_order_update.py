...
class JobOrderUpdate(Document):
	def on_submit(self):
		if self.status=="6. Pekerjaan Selesai":
			...
			content = {"to":"/topics/{}".format(self.principle.replace(" ","_")) , "notification":{"title":self.job_order,"body":"Telah Selesai Oleh {}".format(self.vendor)}, "data":{"job_order":self.job_order},"sound":"default"}
			...
			content = {"to":"/topics/{}".format(self.vendor.replace(" ","_")) , "notification":{"title":self.job_order,"body":"Telah Selesai"}, "data":{"job_order":self.job_order},"sound":"default"}
			s.post(url=url,headers=header,data=json.dumps(content))
